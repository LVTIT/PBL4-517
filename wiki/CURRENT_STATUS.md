...
- **Nâng cấp tính năng e-commerce phục vụ bề mặt kiểm thử OWASP (2026-09-18):**
  ...
  - **Hoàn tất toàn diện kịch bản OWASP A01 (IDOR), Giao diện Demo & Bộ Bằng chứng Thực nghiệm (2026-09-22):**
    ...
    - Triển khai kỹ thuật **Phòng thủ chuyên sâu (Database-level Scoping)** thống nhất trong [`order-service.ts`](../website/backend/src/services/order-service.ts) cho cả 3 thao tác: Ép điều kiện sở hữu `id + userId` ngay tại tầng truy vấn Database khi ở Secure Baseline (`VULN_IDOR_ENABLED=false`), triệt tiêu hoàn toàn nguy cơ rò rỉ dữ liệu nhạy cảm (PII) lên RAM Node.js.
    - Phát triển giao diện Web trực quan: Trang chi tiết đơn hàng [`OrderDetailPage.tsx`](../website/frontend/src/pages/OrderDetailPage.tsx) hiển thị cảnh báo đỏ **403 FORBIDDEN** khi bị truy cập trái phép hoặc banner cảnh báo rò rỉ khi bật cờ lab; nâng cấp [`OrdersPage.tsx`](../website/frontend/src/pages/OrdersPage.tsx) với nút Xem chi tiết và nút Hủy đơn nhanh cho đơn hàng `PENDING`.
...
- [#10 — Scanner](https://github.com/LVTIT/PBL4-517/issues/10): **Implemented / Verified dual alerts.** Đã hoàn thiện prototype scanner đa luồng (ThreadPoolExecutor), cơ chế lưu Baseline và phát hiện cổng mới mở thêm (`scanner_state.json`), tích hợp module `alerts/notifier.py` gửi cảnh báo đồng thời về Discord Webhook (kênh `# scanner`) và Telegram Group Bot (`PBL4-517 Alerts`). Đã kiểm chứng thực nghiệm thành công cả 3 sự kiện (`INITIAL_SCAN`, `NEW_PORT_DETECTED`, `PORT_CLOSED`).
